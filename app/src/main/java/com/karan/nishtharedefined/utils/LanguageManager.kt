...
object LanguageManager {
    ...
    fun setLocale(context: Context?): Context {
        // Also setting the current locale being fetched from Session Preferences
    ...
    fun setNewLocale(context: Context, language: String) {
        ...
        updateResources(
            context,
            language
        ) // The method to decide whether to go for Legacy one or new one
    }
    ...
    private fun updateResources(context: Context, language: String): Context {
        return if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N)
            updateResourcesNew(context, language)
        else
